 ...
 *
 */
Map getAppVersion() {[platform: "SmartThings", major: 2, minor: 0, build: 9801]}
...
def devicePage()
{
	...
	dynamicPage(name: "devicePage", uninstall: false, install: false, nextPage: "mainPage")
	{
		...
		if (requiredDrivers?.size())
		{
			...
			{
				paragraph "Please make sure the following native drivers are installed on the ${getConnectedInstanceType()} hub before clicking \"Done\": \n${requiredDrivers}"
			}
		}
	}
}
